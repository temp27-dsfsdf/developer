document.addEventListener("DOMContentLoaded", () => {
    ...
    const TRANS_CONFIG_JSON = 'https://cdn.jsdelivr.net/gh/temp27-dsfsdf/developer@main/trans_config.json';
    
    Promise.all([
      fetch(WEB_TEMPLATE_JSON_URL)
        .then(response => {
          if (!response.ok) {
            throw new Error("Network response was not ok for template JSON");
          }
          return response.json();
        }),
      fetch(TRANS_CONFIG_JSON)
        .then(response => {
          if (!response.ok) {
            throw new Error("Network response was not ok for trans config JSON");
          }
          return response.json();
        })
    ])
    .then(([template_json, trans_json]) => {
      console.log("불러온 template_json 데이터:", template_json);
      console.log("불러온 trans_json 데이터:", trans_json);

      if (template_json && trans_json) {
        initDCSitemap(template_json, trans_json);
      }
    })
    .catch(error => {
