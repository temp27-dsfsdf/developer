    ...
    <div id="carousel" class="carousel-inner">
      {{#each images}}
        <div class="carousel-item">
          <img src="{{this}}" alt="Carousel Image">
        </div>
      {{/each}}
    </div>
...
// Handlebars 컴파일
const template = Handlebars.compile(carouselTemplate);
const carouselHTML = template(carouselData);

// MAIN_PAGE 감지 후 특정 영역에 삽입
function insertCarouselOnMainPage() {
  const BASE_URL = "https://mcp.dxentric.co.kr:4433/shop/";
  const CURRENT_URL = window.location.href.split("?")[0];
  
  if (new RegExp(`${BASE_URL}index\\.dg$`).test(CURRENT_URL)) {
    const targetElement = document.querySelector("body > section.subscribe_area.section_padding");
    if (targetElement) {
      targetElement.insertAdjacentHTML("beforeend", carouselHTML);
      initCarousel();
    }
  }
}

// MAIN_PAGE onActionEvent에 추가
const MAIN_PAGE = {
  ...
  onActionEvent: () => {
    insertCarouselOnMainPage();
  }
};

document.addEventListener("DOMContentLoaded", insertCarouselOnMainPage);
